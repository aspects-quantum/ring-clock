...
def notick_evolution(times,L,rho0,full=False):
    """
    This is a wrapper for the scipy function scipy.integrate.solve_ivp using the implicit
    BDF (backward differentiation formula) solver.

    Input:
        times   :   numpy array with times at which to evaluate the simulation
        L       :   lindblad superoperator in numpy array form, must be NON-trace-preserving
        rho0    :   initial state
        full    :   False: returns only time and prob, True: returns also density matrix
    Output
        times   :   array of the times at which the solution was evaluated
        notickP :   probability array that the clock did not yet tick at that time   
    """
    # Extract shape of the initial state
    stateshape = rho0.shape
    ...
    res = solve_ivp(temp_func,(min(times),max(times)),np.ravel(rho0),t_eval=times,method="BDF",rtol=1e-6,jac=L)

    notickP = np.zeros_like(times)

    states = np.zeros((len(times),*stateshape))

    for k in range(len(times)):
        notickP[k] = np.real(np.trace(np.reshape(res.y[:,k],stateshape)))
        states[k,:] = np.reshape(res.y[:,k],stateshape)

    if full:
        return res.t, notickP, states
    else:
        return res.t, notickP
